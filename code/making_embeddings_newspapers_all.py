...
def iter_load_sentences(start_year, end_year, data_path):
    ...
    dirs = [item for item in os.listdir(data_path) if os.path.isdir(os.path.join(data_path, item)) if item[0] != '.']
    year_paths = []
    
    for year in range(start_year, end_year + 1):
        for dir in dirs:
            year_path = os.path.join(data_path, dir + '/' + dir + '_' + str(year) + '.txt')
            if os.path.isfile(year_path):
                year_paths.append(year_path)
            yield iter_file(year_paths)
